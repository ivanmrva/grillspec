            ...
            with zipfile.ZipFile(archive) as bundle:
                for info in bundle.infolist():
                    ...
                    if "__pycache__" in Path(name).parts or name.endswith((".pyc", ".pyo")):
                        fail(f"{archive.name}: generated bytecode is archived at {name}")
...
bytecode = sorted(path.relative_to(ROOT) for path in DIST.rglob("*")
                  if path.name == "__pycache__" or path.suffix in {".pyc", ".pyo"})
if bytecode:
    fail("generated release surface contains bytecode/cache artifacts: " +
         ", ".join(map(str, bytecode[:10])))
