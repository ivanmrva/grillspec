...
- stable IDs (bare type prefix): aggregate `AGG-`, command `CMD-`, event `EVT-`, value object `VO-`, entity `ENT-`, policy/saga `POL-`, read-model `RM-`, hotspot `HOT-`. **Write each element `<ID> <Name>` — the ID leads, then its PascalCase name** (e.g. `commands: CMD-201 ExtractAtoms · CMD-204 ReExtract`; or a table row keyed on the ID) — so every element registers as a definition. Namespace contexts by **disjoint numeric bands** (e.g. orders `AGG-2xx`, billing `AGG-3xx`), never with a `<CTX>-` prefix
