...
| `prerequisites.md` | the human-only bootstrap prerequisites — accounts/billing/quotas · secret/credential names + where set · OAuth/app regs · DNS · region/residency choices. **Per item: name + location + the exact provisioning steps + where to inject — never the value** | prose: per-item step-by-step actions |
