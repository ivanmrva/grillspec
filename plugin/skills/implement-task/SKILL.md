...
## Process
1. **Branch + open the Verification Record.** Off main (`task/T-NNN-…`); never commit to main. Generate the pre-implementation checklist — `python3 ${CLAUDE_PLUGIN_ROOT}/tools/check_task_record.py spec --init T-NNN` — which materializes, from the task's frozen spec references, the obligation table you'll be held to. You see the bar before you write a line; you can't shrink it.
