...
- **Give it its input.** Look up the area in the **machine dependency graph** `${CLAUDE_PLUGIN_ROOT}/grill-shared/dependencies.json` (`consumes` = the upstream areas it builds from, `produces_ids` = the IDs it mints; rendered for humans at `${CLAUDE_PLUGIN_ROOT}/docs/DEPENDENCY-GRAPH.md`). Gather exactly those upstream artifacts (by stable ID) and hand them over as its input. It uses what you give it; it never reaches for a required set itself, and it works behind no gate - **you** decide readiness and sequence, it just produces.
