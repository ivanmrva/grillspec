...
1. **Run `${CLAUDE_PLUGIN_ROOT}/tools/lint_spec.py`** — its ERRORs are the authoritative structural truth
   ...
   linter can't see — including a **timeless-source-of-truth sweep**: the spec describes the system as it is now, so strip any development-trace / changelog language that slipped into an area (content tagged `new` / `newly added` / `now` / `previously` / `expand→contract`, notes of what an edit added/cut/deferred, `this round` / `as discussed`), routing the fix to the owning area; the same word is fine as domain language (a `new booking`). 2. **Re-derive** cross-area state incl. cross-references; flag downstream of any
