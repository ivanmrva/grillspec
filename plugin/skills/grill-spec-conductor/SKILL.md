...
The map below is the human overview; the **machine-readable** edges (each area's exact `consumes`/`produces_ids`) live in `${CLAUDE_PLUGIN_ROOT}/grill-shared/dependencies.json` — consult it when handing a skill its input (rendered at `${CLAUDE_PLUGIN_ROOT}/docs/DEPENDENCY-GRAPH.md`).
```
