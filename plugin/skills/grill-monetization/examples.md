...
- `pricing.md` — **Starter** $0.018/min, English-only (fence: language pack), persona = solo dev, 2k free min/mo; **Pro** $0.012/min, all 12 languages + diarization (fence: language pack + priority queue), persona = Series-A product team; **Scale** $0.008/min, volume committed (fence: 250k-min/mo commit), persona = contact-center platform. Discovery basis: acceptable-price corridor $0.006–$0.020/min from 14 WTP interviews; Pro sits at the elasticity knee (~$0.012). Competitor benchmark $0.015/min (one input only).
