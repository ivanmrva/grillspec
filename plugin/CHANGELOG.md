...
### Added — machine-checkable credential provisioning: the `_provisioning.md` register + the afk-eligibility fitness gate
Field feedback: task manifests asserted `afk: eligible` while their `human-prereq` named an external credential that nothing had actually provisioned (`arrives via T-001` — free prose T-001 never delivered), so an unattended run would ambush itself mid-slice on a credential that was never wired, and no single place held the live provisioned-vs-pending truth. Closed with a first-class register + a static gate:
- **`_provisioning.md` — a new spec-root, layer-free, authored register.** One row per external credential/config key (the key matches `infra-ops/environments.md`) → owner (a provisioning `T-` or `human`) · consuming tasks (`T-`) · `state` (`provisioned`|`pending`) · evidence. It is **spec-root on purpose**: it joins a credential (infra, L5) to the delivery tasks (L6) that consume it — a downward map that on a derived L5 file would be an illegal reference — and it carries LIVE state that must survive task re-derivation, so it is authored, not regenerate-only. `derive-tasks` seeds/refreshes the map columns (`state: pending`) **preserving any existing state**; `autorun` reads it BEFORE a wave to park credential-gated branches up front (never discover the block mid-run) and flips a row to `provisioned` when its owner task merges; `audit-spec` treats a `human-prereq` as resolved only when its register row is `provisioned`.
- **`lint_spec.py` check #29 — the fitness gate.** An `afk: eligible` task carrying a non-`N/A` `human-prereq` is a contradiction (the run parks on it). With no register present it WARNs the unbacked claim; with the register present the claim becomes verifiable — every named credential `provisioned` clears it, any `pending`/unregistered credential is flagged. Reads the task-level `afk` from the phase/afk row (so an illustrative `afk: blocked` inside a `prototype-review` cell can't spoof the status). The `afk`-invariant is now stated in `derive-tasks` and wired through the exec engine's escalation path (`_human-input.md` + `bootstrap.md` + a `_provisioning.md` row).

### Changed — `prerequisites.md` rescoped to a pure requirement register; the click-by-click lives only in `bootstrap.md`
`derive-infra-ops` had scoped `prerequisites.md` as the "operationally-complete, current-console step-by-step" — which **duplicated** `12-operate/bootstrap.md` (the operate-layer runbook) and forced an illegal L5→L6 downward reference whenever the register tried to point at "the steps". The two drifted (one grew a console path the other lacked). Now `prerequisites.md` is the **requirement register** — per dependency: name · where-set · the config key(s) it feeds (→ `environments.md`) · phase · audience/owner · residency/compliance constraint — **never a value, never a console click-path**. `bootstrap.md` is the **single home of the click-by-click**, composing the upstream register + config matrix into current-console steps (register states *what*/*who*; runbook is *how*). This also aligned the skill with its own worked example, which had already been register-shaped. Operational "where are the steps" navigation lives in the spec-root `_human-input.md` (layer-free, may cite `bootstrap.md §N`), which a layer-bound solution file may not.

### Added — spec-lint: ADR sequence-gap + dangling ratified-decision checks (#27, #28)
A ratified decision recorded in `_human-input.md` as `Decided (ADR-INFRA-004)` with no `ADR-INFRA-004` file on disk went unflagged — the dangling-ref checks fire only when something *actively* cites the id in a checked layer, and `_human-input` is exempt. Two checks close the gap: **#27** flags interior gaps in each area's ADR numbering (003, 005 filed → missing 004 usually means a decision was made but never filed; archived ADRs count as filed); **#28** cross-checks that every RESOLVED `_human-input.md` decision citing an ADR resolves to a real `adr/` file.

### Fixed — two false positives in the no-fakes / no-fake-deploy tripwires
- **`check_deploy_real.py`** flagged the bare English word `No-op` in a legitimate CI comment (e.g. `# No-op on non-main branches`) as a faked deploy — `no[ _-]?op` was the one placeholder token not qualified with `deploy` like its `fake-deploy`/`stub-deploy`/`dummy-deploy` siblings. Qualified to `no-op deploy`.
- **`check_no_fakes.py`** over-matched two ways: `canned` inside an unrelated word (`scanned`/`scannedKeys` in a config-key builder), and `hardcoded` inside a negation comment (`// …never hardcoded`). `WARN_IDENT` now matches only at a sub-token boundary (identifier start · `_` · camelCase hump), and a negation-comment guard suppresses a hit inside an inline comment that negates the keyword. Real identifiers (`HARDCODED_KEY`, `cannedResponse`, `isPlaceholder`) still fire.

All fixes ship with regression tests locking the precision (checker suites + `test_lint_spec.py`); `selfcheck` green.

## 4.13.0

### Added — `audit-spec --fix` / `--loop`: the audit's fix-chains become executable, and the spec can be driven to zero findings
The audit routed every derived-zone defect correctly on paper ("edit upstream → re-derive") — then wrote the chain into `spec-audit-report.md` and orphaned it: nothing executed the report, so fixes landed upstream while `10-delivery` (conventions · tasks · impl-design · root `CLAUDE.md`) silently kept encoding the old truth. Two new modes on the same skill (report-only stays the default):
- **`--fix`** — one audit pass, then remediate every finding at its correct fix-zone under an explicit **exit contract**: after the last upstream edit, `impact.py` over the changed IDs → re-run the owning derive step for **every** derived artifact in the set — explicitly through the terminal derived layer (`10-delivery/conventions|tasks|impl-design`, root `CLAUDE.md`), the layer a partial propagation most often strands — → `guard_derived.py --record` + `check_freshness.py --record` → Phase-0 baseline back to green. **Reporting "fixed" while the impact set of your own edits contains an un-re-derived artifact is forbidden** — a computed-but-unprocessed impact set is an unfinished fix.
- **`--loop`** — repeat `--fix` passes until the spec converges to **zero findings including minor**: a parallel two-track audit per pass (Track A: N=3 independent whole-spec blind re-derivations with divergent lenses — entity-lifecycle · rule-collision+edge-generators · persona×JTBD journeys — stochastic, never scoped down; Track B: consistency/structural, scoped on rounds 2+ to the previous round's change set + its reference-graph downstream), **read-only (`Explore`) fan-out auditors** (only the orchestrating loop edits, after grounding each finding), seen-set dedup + a carried do-not-re-flag list, the propagation tail drained in-round, locks re-recorded, the report deleted per pass, and a convergence gate of **K=2 consecutive dry batches** + a final clean full sweep. Protocol lives in `remediation-loop.md` (loaded only in loop mode), ported and generalized from a field-proven project-local harness — including its incident-earned guardrails (write-capable auditors once contaminated a working tree mid-audit: revert + re-apply; hence read-only fan-out, and one loop per repo).
Consolidated as modes on `audit-spec` rather than a separate looping skill: independence lives at the agent level (fresh read-only auditors each round), a separate skill would have to name-invoke its sibling (only the conductor names skills), and one file can't drift from the phases it wraps.

### Added — `audit-build`: multi-lens judgment fan-out + the re-audit cycle (and why it gets NO loop)
The whole-spec audit loops because its Phase-3 blind-spot space is unbounded — repeated independent sampling keeps paying. The build audit's object is **finite and enumerable** (the done `T-` records, the test tree, the ledger): enumeration (`--deep`) dominates repeated sampling, so no convergence loop — but the per-item judgments (is this `VERDICT` rubber-stamped? would this test fail if the behavior broke? did a later task weaken an earlier guarantee?) carry model-judgment variance, and that is bought back **within one pass**, not across passes:
- **Parallel read-only fan-out for Phases 1–3** — N=3 `Explore`-type auditors over the same scope with divergent lenses (**evidence-backing** · **test-bites**/rubber-stamp detection · **cross-task drift**), union → dedup → every finding grounded by the orchestrator before it enters the report. Full-scope per lens in `deep`, risk-weighted sample in `attest`.
- **The re-audit cycle, stated** — and a hard "no `--fix`/`--loop`, by design" rule: the attestor cannot fix what it attests (editing a record to satisfy the audit is the fraud Phase 1 hunts), and build fixes run through the engineering workflow (rework `T-` → implement → test → conformance → CI → merge), asynchronous by nature. Cycle: `NOT-ATTESTED` + routed findings → rework waves land → a **fresh** audit (the `commit:` stamp + the release gate's `--require-fresh` already invalidate a stale attestation mechanically). Re-audits may scope Phase-1 re-checks to tasks touched since the last report; Phases 2/4 always run full; the release-gating `ATTESTED` is always a fresh, full `--deep`.

### Fixed — the propagation duty was claimed but nowhere carried (the root cause of stale conventions/tasks)
- **The conductor's §Change propagation asserted "any skill that makes a change owns steps 1–5; the engines bake this in" — no engine carried any of it**, so a directly-invoked skill could edit upstream and end with every link resolving and every derived artifact stale. The false claim is gone, replaced by the real split — **kept OUT of the shared engines on purpose**: engines ship inside standalone skills used outside any grillspec project, so they stay system-blind (a worker only names the changed IDs in its hand-off and never chases dependents). **Propagation belongs to the system-aware composers**: the conductor when driving, and `audit-spec --fix`/`--loop` under their exit contract.
- **The net for conductor-less sessions moved to the composer's door**: the conductor's entry scan now runs `check_freshness.py` beside the linter (advisory — every artifact whose cited upstream drifted since last reconciliation), and its option menu gains **drain the stale set** (re-grill/re-derive each drifted artifact via its owning skill, upstream→downstream through `10-delivery`, then re-record the locks). A direct invocation may leave the tree stale; the next composed session catches and drains it.
- **The audit report was terminal** — the conductor's menu now offers `--fix`/`--loop` on the audit option, and when a `spec-audit-report.md` already sits at the project root, an **"execute the audit's fix-chains"** option (`audit-spec --fix` consumes it): the report's dependency-ordered chains are work to drive, not an artifact to admire.
- **Standalone-purity sweep across all 46 worker skills + the shared engines** — worker skills ship individually (no tools, no siblings, no conductor), so they must carry zero awareness of the composed system. A parallel four-agent review found and fixed 12 leaks: sibling skills named from workers (`derive-conventions`/`grill-compliance`/`migrate-data`/`autorun` referenced by name from derive-tasks, grill-data-reqs, deploy-release, conformance-review, generate-ui-prototype, derive-tasks) → generic role phrases; "framework-provided"/"framework-level"/"the exec engine" actor references in derive-tasks/derive-conventions/derive-infra-ops → neutral wording; the exec engine's step-zero gate arming made conditional on the toolset being present, with an explicit standalone rule (every discipline still binds without the mechanical backstops — never fake a gate's output or claim a check you couldn't run, never remove or ignore a gate the project carries). The allowed system-aware set is exactly: the conductor, `audit-spec`, `audit-build`, `autorun`.
- **The standalone emitters dropped skill resources** — both hard-coded `SKILL.md`+`examples.md`, so any other sibling resource (like `remediation-loop.md`) silently never shipped in `dist/skills/` or cluster plugins. Both now copy every `*.md` in the skill folder.

## 4.12.0

### Added — FINAL code + real tests enforced from the very first line (the bootstrap window is now gated)
Field feedback: on real projects the earliest setup tasks still snuck skip flags, mocks, and assumption-driven workarounds into code and tests "to finish the task" — banned by the engine's prose, caught by no tool, and worst exactly in the window where the least governance was live. Four layers close it:
- **`check_no_skips.py` — the disabled/weakened-test tripwire** (the test-side sibling of `check_no_fakes.py`; + `test_check_no_skips.py`, and a scenario in the `test_e2e_gates.py` composition chain). The anti-cheat rule "never add `skip`/`xfail`/`ignore`, never shrink the suite" lived only in prose. Now mechanical — ERROR: a skip/disable marker across ecosystems (`it.skip`/`describe.skip`/`xit`/`this.skip()` · `@pytest.mark.skip/skipif/xfail` · `pytest.skip/xfail/importorskip` · `skipTest` · Go `t.Skip` · Rust `#[ignore]` · JUnit `@Disabled`/`@Ignore` · xUnit `[Ignore]`/`Skip=""` · PHPUnit `markTestSkipped` · Ruby `skip`/`pending`), a suite-shrinking focus marker (`.only`/`fit`/`fdescribe`), and a CI/build file that swallows a red test run (`npm test || true`, gradle `ignoreFailures = true`, maven `testFailureIgnore`). WARN, promoted under `--strict` (which audit-build passes at release): declared deferrals (`it.todo`, `markTestIncomplete`), `continue-on-error:`/`allow_failure:`, `--passWithNoTests`, Go `testing.Short()`. Waive the rare legit case with an inline `no-skips: allow <reason>` or `.claude/no-skips-allow.txt`. Wired into: the exec-engine done-gate, derive-conventions' pre-commit hook set + tier-contract rule, conformance-review Lens A, run-tests' never-pass list, and audit-build Phase 0.
- **`gate_exec.py` gains gate 3 — the cheat is denied at the keystroke.** A Write/Edit that INTRODUCES a skip/xfail/`.only` marker into a test file, or a mocking-library import / `Fake*` double definition into `src/`, is denied at PreToolUse time — mid-task is where the rationalization happens ("the broker isn't ready, skip it for now"), and by commit time code is built on top of it. Task-INDEPENDENT on purpose: it fires during setup/bootstrap work before any task branch exists. Only newly-introduced lines trigger (line-set diff vs the replaced text); inline waivers pass; fail-open and `GRILLSPEC_GATE_OFF=1` unchanged.
- **Step zero — the exec engine arms the gates at session entry, before ANY task.** `install_exec_gates.py` used to run as a T-001 deliverable, so the setup tasks — precisely the reported cheating window — ran with no gate live. Every exec session now runs the (idempotent) installer as its first action; the walking skeleton still commits the wired `.claude/settings.json` + vendored tools so worktrees and later sessions inherit them.
- **A red suite over a real implementation is a first-class, reportable end-state.** New anti-cheat invariant (exec-engine) + implement-task rule: when a prerequisite genuinely isn't ready, real code + real failing test + `status: blocked` + the escalation record is process **success** — the agent is never cornered into green-at-any-cost. conformance-review additionally requires the e2e **not-exercised enumeration**: every referenced `AC-`/flow branch the e2e leaves untouched needs a stated reason in the review report — assumption-driven path avoidance ("X might not be ready, steer around it") is a blocking violation exactly like a skip; forcing the enumeration catches the weakened-but-green e2e that "don't avoid paths" prose never did. derive-conventions' generated root `CLAUDE.md` must carry the three-line **final-code-only block** (no fakes outside `tests/` · no skip/xfail/`.only` · red-on-unready-dependency is correct, report it) — the one prose net present in sessions that never load a skill.

### Fixed
- **`check_task_record.py` read a bar keyword inside ordinary words.** The bar-detection regex had no word boundary, so `min` matched inside 'deter**min**istic' and a stray number became the bar — false-failing a passing coverage row (measured 93.8 vs the real 80 bar; hit on a done-flip in the field). Word keywords (`bar`/`threshold`/`target`/`min(imum)`/`floor`) are now `\b`-anchored; the `>=`/`≥` symbol forms stay unanchored. Regression tests lock both directions (substring must pass; a genuinely-below-bar row with such a word must still fail).
- **`tier_contract.py` didn't classify camelCase test names** (`PayTest.java`, `PayTests.cs`, `PaySpec.kt`) — JVM/C#/PHP suites were invisible to every tier gate. Recognized now, case-SENSITIVE on purpose: a case-insensitive match would classify a production `Latest.java` as a test and blind the gates to it.
...
- **`audit-build` — the whole-build audit**, the code-plane mirror of `audit-spec` (`audit-build` : the per-task conformance review :: `audit-spec` : the linter). The per-task conformance review is inline, self-certifying QA; this is the independent, release-time **external audit** that distrusts the accumulated verdicts. It owns the three things no per-task review can see: **ledger integrity** (did every gate genuinely run; is every recorded `VERDICT: PASS` backed, or rubber-stamped), **cross-task emergent** defects (an `AC-` whose covering test a later task weakened; suite shape vs the declared distribution; aggregate coverage/mutation), and **operate-ledger reconciliation** (`12-operate/` records vs the spec they enacted). Verdict `ATTESTED`/`NOT-ATTESTED`, gated behind a clean `audit-spec`. Depth `attest` (sampled) / `deep` (full re-judgment).
- **Machine-readable tier contract in the test strategy.** `derive-test-strategy`'s `levels.md` now emits a per-tier row (`tier · real-deps · may-mock · mock-ceiling · target-env · coverage-bar · mutation-bar`) with the best-practice default spelled out — the ground truth the new verifiers read. A mock/real policy in prose can't be checked; the contract makes "over-mocked" and "wrong-tier" mechanical.
- **Four new deterministic checks** (each with a regression suite; run in the project pre-commit + `code-ci.yml` and read at aggregate scope by `audit-build`): `check_mock_budget.py` (no mock beyond a tier's ceiling — the complement to `check_no_fakes.py`, which never scans `tests/`), `check_test_tiers.py` (every declared tier has a suite; `--require` makes a test tree with no contract a hard error; surfaces untier-able files so the gate can't go silently blind; detects tiers by directory OR filename), `check_e2e_target.py` (no "e2e" on a local stack), `check_operate_records.py` (the only reconciler for the append-only operate layer). Wired into `derive-conventions` (project hooks) + the exec-engine done-gate.
- **`check_release_attestation.py` — an opt-in release gate**: a `build-audit-report.md` must read `ATTESTED` before release. The release bar is a **`--deep`** audit (a full independent re-judgment, not a sampled `attest` pass — which can miss the one task that overfit its visible tests, the real risk in solo builds where no `AC-` is held out); `--require-deep` enforces it. It also enforces **freshness**: the audit stamps `commit: <sha>` and the gate rejects a STALE attestation (HEAD moved since the audit) — `--require-fresh` additionally rejects an unverifiable one. Ships available, wired-on only if the team makes it a required check (the per-task checks aren't optional; the release decision stays the team's).
- **Mutation/coverage gate hardened** (`check_task_record.py`): a present, non-`N/A` coverage/mutation row must cite a checkable measured value AND its bar (`bar`/`threshold`/`target`/`≥`) — free-form evidence ("looks good") is how the gate got silently bypassed; a **bare `N/A` with no reason is also rejected** so the assertion-quality gate can't be silently skipped.
- Tests: `test_check_mock_budget` · `test_check_test_tiers` · `test_check_e2e_target` · `test_check_operate_records` · `test_check_release_attestation`; `test_e2e_gates` extended so all gates compose on one realistic project (each cheat trips only its owner).
...
- **The exec-gate joins the build-accountability catalog.** `gate_exec.py` was a sibling of `check_task_record.py`/`check_no_fakes.py` but wasn't listed in `HOW-IT-WORKS.md` (the gate table) or `SPEC-CHECKS.md` (the build-accountability section). Added to both, flagged as the only enforcer that runs **at tool-call time** (PreToolUse) — a distinct enforcement point from the pre-commit/CI gates. (`audit-spec` deliberately still omits it: that skill is the *spec-layer* judgment, and exec/code gates aren't in its inventory by design.)
- **`.gate/` self-ignores.** The active-task pointer + red-logs under `spec/10-delivery/verification/.gate/` are transient per-session build state; `gate_exec.py` now drops a `.gate/.gitignore` (`*`) when it first writes, so they're never committed (and can't churn or conflict). `lint_spec.py` already tolerates the dir (it sits under a leaf dir, so the closed-world check allows it and the `.md`-only scans skip it) — verified, no linter change needed.
...
- **`install_exec_gates.py` — the idempotent installer the walking-skeleton runs.** Vendors `gate_exec.py` + `check_task_record.py` into the project and merges the PreToolUse hook into `.claude/settings.json`, preserving the user's other keys/hooks; no-ops if already present. **Refuses to wire a hook it can't back** — a PreToolUse command pointing at a missing file exits non-zero, which would *block* every edit, so it bails (no settings written) unless the script is in place. Project-local like the git pre-commit hook — it writes only **this** repo's `.claude/`, never `~/.claude`, and never fires on other projects.
