...
## How to run it
- **In the bundle plugin:** `/grillspec:grill-security-reqs` in Claude Code or `$grillspec:grill-security-reqs` in Codex
- **Standalone:** copy the `grill-security-reqs/` folder into `~/.claude/skills/` and run `/grill-security-reqs` in Claude Code, or into `~/.agents/skills/` and run `$grill-security-reqs` in Codex. It works on its own and composes with sibling skills, each writing to its own output folder.
