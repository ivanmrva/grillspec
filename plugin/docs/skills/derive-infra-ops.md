...
## How to run it
- **In the bundle plugin:** `/grillspec:derive-infra-ops` in Claude Code or `$grillspec:derive-infra-ops` in Codex
- **Standalone:** copy the `derive-infra-ops/` folder into `~/.claude/skills/` and run `/derive-infra-ops` in Claude Code, or into `~/.agents/skills/` and run `$derive-infra-ops` in Codex. It works on its own and composes with sibling skills, each writing to its own output folder.
