...
## How to run it
- **In the bundle plugin:** `/grillspec:derive-ml-architecture` in Claude Code or `$grillspec:derive-ml-architecture` in Codex
- **Standalone:** copy the `derive-ml-architecture/` folder into `~/.claude/skills/` and run `/derive-ml-architecture` in Claude Code, or into `~/.agents/skills/` and run `$derive-ml-architecture` in Codex. It works on its own and composes with sibling skills, each writing to its own output folder.
