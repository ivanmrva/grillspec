...
## How to run it
- **In the bundle plugin:** `/grillspec:grill-ddd` in Claude Code or `$grillspec:grill-ddd` in Codex
- **Standalone:** copy the `grill-ddd/` folder into `~/.claude/skills/` and run `/grill-ddd` in Claude Code, or into `~/.agents/skills/` and run `$grill-ddd` in Codex. It works on its own and composes with sibling skills, each writing to its own output folder.
