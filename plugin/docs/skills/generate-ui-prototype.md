...
## How to run it
- **In the bundle plugin:** `/grillspec:generate-ui-prototype` in Claude Code or `$grillspec:generate-ui-prototype` in Codex
- **Standalone:** copy the `generate-ui-prototype/` folder into `~/.claude/skills/` and run `/generate-ui-prototype` in Claude Code, or into `~/.agents/skills/` and run `$generate-ui-prototype` in Codex. It works on its own and composes with sibling skills, each writing to its own output folder.
