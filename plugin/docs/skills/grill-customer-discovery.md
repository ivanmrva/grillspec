...
## How to run it
- **In the bundle plugin:** `/grillspec:grill-customer-discovery` in Claude Code or `$grillspec:grill-customer-discovery` in Codex
- **Standalone:** copy the `grill-customer-discovery/` folder into `~/.claude/skills/` and run `/grill-customer-discovery` in Claude Code, or into `~/.agents/skills/` and run `$grill-customer-discovery` in Codex. It works on its own and composes with sibling skills, each writing to its own output folder.
