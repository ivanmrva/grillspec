...
## How to run it
- **In the bundle plugin:** `/grillspec:grill-constraints` in Claude Code or `$grillspec:grill-constraints` in Codex
- **Standalone:** copy the `grill-constraints/` folder into `~/.claude/skills/` and run `/grill-constraints` in Claude Code, or into `~/.agents/skills/` and run `$grill-constraints` in Codex. It works on its own and composes with sibling skills, each writing to its own output folder.
