...
## How to run it
- **In the bundle plugin:** `/grillspec:derive-security-architecture` in Claude Code or `$grillspec:derive-security-architecture` in Codex
- **Standalone:** copy the `derive-security-architecture/` folder into `~/.claude/skills/` and run `/derive-security-architecture` in Claude Code, or into `~/.agents/skills/` and run `$derive-security-architecture` in Codex. It works on its own and composes with sibling skills, each writing to its own output folder.
