...
## How to run it
- **In the bundle plugin:** `/grillspec:run-tests` in Claude Code or `$grillspec:run-tests` in Codex
- **Standalone:** copy the `run-tests/` folder into `~/.claude/skills/` and run `/run-tests` in Claude Code, or into `~/.agents/skills/` and run `$run-tests` in Codex. It works on its own and composes with sibling skills, each writing to its own output folder.
