...
## How to run it
- **In the bundle plugin:** `/grillspec:grill-goals` in Claude Code or `$grillspec:grill-goals` in Codex
- **Standalone:** copy the `grill-goals/` folder into `~/.claude/skills/` and run `/grill-goals` in Claude Code, or into `~/.agents/skills/` and run `$grill-goals` in Codex. It works on its own and composes with sibling skills, each writing to its own output folder.
