...
## How to run it
- **In the bundle plugin:** `/grillspec:grill-design-system` in Claude Code or `$grillspec:grill-design-system` in Codex
- **Standalone:** copy the `grill-design-system/` folder into `~/.claude/skills/` and run `/grill-design-system` in Claude Code, or into `~/.agents/skills/` and run `$grill-design-system` in Codex. It works on its own and composes with sibling skills, each writing to its own output folder.
