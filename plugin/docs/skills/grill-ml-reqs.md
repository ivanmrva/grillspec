...
## How to run it
- **In the bundle plugin:** `/grillspec:grill-ml-reqs` in Claude Code or `$grillspec:grill-ml-reqs` in Codex
- **Standalone:** copy the `grill-ml-reqs/` folder into `~/.claude/skills/` and run `/grill-ml-reqs` in Claude Code, or into `~/.agents/skills/` and run `$grill-ml-reqs` in Codex. It works on its own and composes with sibling skills, each writing to its own output folder.
