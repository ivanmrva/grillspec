...
| **Structural coverage** (gap surface) | every `CMD-`→`UC-`→`AC-`→`T-` · `EVT-` has a consumer · `OBL-`→control · **`THR-`→`SEC-`** · **`INV-`→`AC-`** (an invariant is asserted by an AC, or enforced structurally — by construction / a `DATA-` constraint) · `NFR-`→`ASR-`/`SLO-` · `API-`→consumer | WARN |
...
`test_check_mock_budget.py` · `test_check_test_tiers.py` · `test_check_e2e_target.py` ·
