...
**Consider every upstream layer, not just your named inputs.** Everything produced in earlier layers (foundation → domain → functional → requirements → …), plus any same-stage output whose derive-order is fixed before yours, is **available context** — read what's relevant. Your listed inputs are the **primary, "start-here" sources**, not an exhaustive or gating set: a requirement legitimately references anything strictly upstream. So don't treat a fact's absence from your named inputs as permission to ignore it if it sits in an earlier layer — and **cite the IDs you actually use**, because change-propagation is computed from real references, not from a declared list.
