...
Code reveals edge cases no model anticipated — expected, not a failure. When you hit an **undecided behavior** mid-implementation, **stop, don't guess**: record a `GAP <what's undecided> — <owning area> — UNRESOLVED` on the task and resolve it just-in-time one of three ways — **complete it** (elicit the rule/UX — it lands in the source spec), **mark N/A** for this slice, or **accept a default** (an ADR in `adr/`). The task is implementation-final again only once the gap is resolved. Never encode an invented behavior to keep moving.
