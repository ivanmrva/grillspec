...
## Process
1. **Select the ready wave.** From `10-delivery/tasks/build-order.md`, take every task that is **AFK-eligible** (`afk: eligible` — no HITL trigger, no unresolved gap) **and** whose `depends:` are all merged. **Cross-check the credential register (`_provisioning.md`) FIRST, before dispatching** — a task whose `human-prereq` names a credential that is `pending` (or absent from the register) is **not actually ready**: park it upfront with the precise missing credential, rather than dispatching it to discover the block mid-slice and burn a run. (`lint_spec.py` flags this `afk:eligible` + un-provisioned contradiction statically; autorun is the runtime gate.) **Also cross-check the prototype-review gate:** a task with a non-`N/A` `ux` dimension whose prototype is **not frozen (human-reviewed)** and which carries no explicit `prototype-review: waived — <why>` is **not ready** — park it `blocked — visual/UX review pending` rather than letting a JIT-generated, never-seen screen build unattended (that is the divergence the frozen-prototype window exists to prevent). The intended cadence is **batch prototype-finalization before the wave**: generate + click-through + freeze every ux-heavy slice queued for the wave in one attended sitting, then dispatch — autorun stays unattended, the visual judgment happens before code, not as after-the-fact rework.
2. **Run the wave in parallel.** Prefer the active host's managed parallel workflow for the whole wave; otherwise use one subagent or headless process (`claude -p` or `codex exec`) per task. **Each task runs the full done-gate loop** — implement → run the whole gate → self-correct the *code* → repeat until green — honoring the **anti-cheat invariants** (fix code not goalposts · spec is upstream truth · green ≠ done · never disable a gate).
...
- **The tool-call-time exec-gate enforces every task in the wave, independently** — `gate_exec.py` (red-before-green · no hollow done-claim) keys the active task off each task's **`task/T-NNN` branch**, so it is parallel-safe with no shared pointer to clobber. Run each parallel task in **its own branch/worktree** (the workflow already mandates branch-per-task) — the committed `.claude/settings.json`/`.codex/hooks.json` adapters plus the one shared `.grillspec/tools/` tree travel into the worktree, and each worktree's `.grillspec/gate/` red-log is local — so the per-task RED→GREEN cadence is held live in every wave, not just caught at the commit-time gate. (If you dispatch a task *without* a `task/T-NNN` branch, the implementing agent must `gate_exec.py --start T-NNN` for the gate to engage.)
