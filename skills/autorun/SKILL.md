...
- **The tool-call-time exec-gate enforces every task in the wave, independently** — `gate_exec.py` (red-before-green · no hollow done-claim) keys the active task off each task's **`task/T-NNN` branch**, so it is parallel-safe with no shared pointer to clobber. Run each parallel task in **its own branch/worktree** (the workflow already mandates branch-per-task) — the committed `.claude/settings.json`/`.codex/hooks.json` + vendored tools travel into the worktree, and each worktree's `.gate/` red-log is local — so the per-task RED→GREEN cadence is held live in every wave, not just caught at the commit-time gate. (If you dispatch a task *without* a `task/T-NNN` branch, the implementing agent must `gate_exec.py --start T-NNN` for the gate to engage.)
