...
**Load `references/grill-engine.md` first and follow it.** This skill applies that method to **Security & privacy requirements** — the threat model, not the mechanism.
...
|---|---|---|
| `authorization.md` | authorization model — one rule per row, **columns `SEC-id · actor · command · decision · condition`** (decision = allow/deny; DEFAULT-DENY, so **every command (CMD-) carries an explicit rule** and no cell is blank), condition holds the ownership/tenant predicate; model shape (role/attribute/relationship-based) noted, decided in an ADR. At build time a `SEC-` rule binds the implementing slice via its `security` dimension, where the done-gate requires the **deny path exercised by a test** — enforcement middleware that exists but is never invoked on the deny branch is unevidenced | rule table (SEC- · actor · command · decision · condition) |
...
| `privacy-audit.md` | privacy/compliance obligations (lawful basis · data-subject rights) + audit requirements (what's logged · append-only · retention); privacy threats may land here instead of `threat-model.md` — **but they keep their `THR-` ids wherever they land** (this file is an alternate location, never an ID-free zone — an un-id'd privacy threat escapes the THR-coverage check), and each stated obligation cites the `OBL-`/`SEC-`/`DATA-` id that owns it | `THR-` rows + `OBL-`/`SEC-`/`DATA-` refs |
