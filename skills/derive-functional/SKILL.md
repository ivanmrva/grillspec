...
**Load `references/derive-engine.md` first and follow it.** This skill applies that method to **Functional spec** — use-cases plus testable acceptance criteria, projected from the domain model.
...
- **every use-case projects its happy path PLUS the alternate/exception flows its command's invariants and policies entail NOW** — they fall straight out of the projection, never deferred
- every use-case has acceptance criteria traceable to a domain-model rule (or a logged gap)
