...
**Shared rules:** load `references/house-rules.md` (the workspace-only source fence · timeless artifacts · output discipline · ID mechanics · plugin-feedback routing) and `references/decision-classes.md` (decide vs. escalate — apply it to every fork; ASK only a fact the user/org/platform holds that the spec can't carry).
...
**ID definition semantics.** The tokenizer knows the ID grammar (suffix starts with a digit or uppercase; dotted members on `DATA-` only; an ADR reference resolves to its numeric core even cited as a file link) — write naturally. Two semantic rules still need care:
