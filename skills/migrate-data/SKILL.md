...
**Load `references/exec-engine.md` first and follow it.** This skill: **migrate data (schema/data evolution)** — forward + rollback, idempotent, online-safe, verified.
...
- **a PRODUCTION cutover is an owner go/no-go even when non-destructive** — the read-switch / dual-write→switch / "run it against prod now" moment needs an **explicit owner go** (or a once-ratified auto-cutover policy): verification (step 3) is required but **not** authorization to proceed in prod. Recommended default to ratify: auto-run in dev/stage, require an owner go for the prod cutover
