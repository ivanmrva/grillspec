...
**Load `references/grill-engine.md` first and follow it.** This skill applies that method to **UX requirements** — journeys, per-role information needs, the information architecture, and accessibility/i18n. The design system (tokens · components · brand · voice) is a **separate area**; journeys reference it as a given input.
...
- **every journey cites the `UC-` use-case(s) it renders, by id** — the journey is the rendering of a use case; one with no use-case basis is a gap, never a requirement invented here (mirrors how an `AC-` keys to its `UC-`). **Exception:** a journey rendering a **Generic, deliberately-unmodeled subdomain** (passwordless auth, tier/billing management — identity/billing kept Generic, not modelled as use-cases) has no `UC-` to cite and declares **`N/A — renders <Generic subdomain>, unmodeled`** instead of mis-citing one
