...
**Load `references/grill-engine.md` first and follow it.** This skill applies that method to **Data requirements** — data governance, not storage design.
...
- **retention = a TTL + a deletion trigger**, never 'a while' ('deleted on \<obligation\>', 'kept ≥N yrs because \<domain reason\>', 'regenerable → N days') — and the slice that owns a `DATA-` class's purge/deletion path must **exercise that trigger in its tests** (the `data` obligation row), so a spec'd retention isn't an unimplemented promise
- **residency (where data is physically stored) is split from sovereignty (whose law governs / who can lawfully compel access)** — both set per class, **consuming the operating footprint that the compliance area already confirmed (don't independently re-set or invent it)**; where the footprint or a retention value is genuinely a fresh call, it's a ratify-point — propose a recommended value (+ one-line why) for the human to agree/override, never silently filled
- **lineage/provenance per class** — origin system · derivation · downstream consumers, **naming the `AGG-`/`DATA-` id wherever the origin or a consumer is a modelled aggregate/data-class** (a typed id, not a prose name) — so a deletion cascade is mechanically answerable (delete here, what else must follow)
