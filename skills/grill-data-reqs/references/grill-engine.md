...
**Shared rules:** load `references/house-rules.md` (the workspace-only source fence · timeless artifacts · output discipline · ID mechanics · plugin-feedback routing) and `references/decision-classes.md` (the decide-vs-ask convergence test — apply it to every fork).
...
For each: **always lead with a concrete recommended value + a one-line why**, never a blank or an open "what should this be?" — the human's job is to agree fast or override, not to author. The lifecycle is a closed set: mark the value **`status: unconfirmed`**; the human flips it to **`ratified`** (agreed) or **`overridden`** (their value replaces yours) — nothing else settles it. Ask inline when interviewing; when running unattended, park it in `spec/_human-input.md` as a ratify-queue row (`| topic/ID | recommended value | why | status |`) so the user clears many in one pass. A recommendation with no concrete value isn't a recommendation. **Propose ≠ decide:** a default the human never saw is an assumption — the linter flags any `unconfirmed` value (an ERROR once tasks exist), and the audit treats it as a finding.
...
A **risk** — something that could go wrong for *delivery*, distinct from a *bet* about whether the idea works — is recorded inline beside what it threatens, with closed value sets so the register reconciles mechanically: `category` ∈ technical · schedule · security · vendor · key-person · technical-debt; `probability` ∈ Low · Medium · High; `impact` ∈ Low · Medium · High · Critical; `owner`; `mitigation`; `risk-status` ∈ open · mitigated · accepted · closed (named `risk-status`, never bare `status` — that word belongs to the bet-validation axis). **Technical debt** (category `technical-debt`) additionally carries a **paydown-trigger** — a debt row without one is incomplete. Whatever composes the areas reconciles these into a register by reading the artifacts.
