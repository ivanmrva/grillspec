...
- **human-only prerequisites enumerated as a first-class REQUIREMENT REGISTER** — one row per external dependency: **name · where it's set (CI · platform · IaC backend) · the config key(s) it provisions (→ `environments.md`) · phase · audience/owner · any residency/compliance constraint — but NEVER the value itself** (a secret's value is never written to the spec). Coverage: accounts + billing/quotas · secret/credential names · OAuth/app registrations · DNS records · account-gated region/residency choices. This register is the **requirement** — *what* must exist and *who* owns it — consumed by the walking-skeleton and by deploy-release; the executable **click-by-click provisioning steps live downstream in the operate-layer `12-operate/bootstrap.md` runbook** (which references this register UPSTREAM), **NOT here** — so the steps have one home and can't drift in two places, and the upstream-only layer rule stays satisfied (a solution-layer register never points down at its operate-layer runbook)
- **the register is complete along its axes, not a click-path** — every external dependency is tagged by **phase** — `initial` (needed to bring up local + dev) · `pre-launch` (production, before go-live) · `later` (day-2: rotation, scale, new regions) — and by **audience/owner** — `ops-admin` (accounts/billing/org policy) · `sys-admin` (DNS/network/secrets store/IaC backend) · `developer` (local env, app config); and states, per dependency, **what must be provisioned in advance vs. what can wait**, what to **capture** (the value to record), **where to store it** (which secret store / CI / platform config), and **how it's authored per environment** (cross-refs `environments.md`). It deliberately carries **no current-console click-path** — that ages out and belongs to the runbook an operator follows (`bootstrap.md`); the register stays the stable, layer-pure statement of the requirement
