...
**Load `references/exec-engine.md` first and follow it.** This skill: **run a task's tests and gate on them** (running them IS the verification).
...
- **Never pass on:** red tests · **a skipped/xfail'd test in the suite** (a skip is a missing test wearing green — `check_no_skips.py` is the mechanical pass; a suite that prints `N skipped` is not clean) · an `AC-` with no test **or tested at the wrong level** (mock-only where a real e2e/integration path is required) · **interaction-only assertions** with no observable-outcome check · an **async/job behaviour not exercised end-to-end** (incl. retry/idempotency/failure) · **a referenced `SEC-` deny rule or `ENTL-` gate whose deny / over-limit / lapsed-billing path no test invokes** (authz middleware that exists but is never exercised on the deny branch is unevidenced) · **a `ux`-cell interaction state with no test, an a11y scan with criticals (or none run), or a screen unverified at the DS breakpoints** (UI slices) · **an `obs`-cell signal (SLO- telemetry · AEV- analytics event) the tests never assert was emitted** · **a `DATA-` retention/TTL/deletion trigger the owning slice never exercises** · **an `ML-` capability with no eval-harness run measured against its stated bar** · a test that **cannot fail** · **a behavioral test with no live spec driver** (an orphan test source or a dangling `@covers` — drift; `check_orphan_tests.py` is the mechanical pass: the behavior's ID is minted/amended in the spec, never a tag invented to appease the gate) · **a changed `src/**` line no test executes, or a diff-mutation score below the bar** (coverage/mutation are diff-scoped — a whole-tree % hides an untested snippet in its slack) · coverage below target. On fail → back to implementation.
