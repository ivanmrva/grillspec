...
**Load `references/exec-engine.md` first and follow it.** This skill: **implement one prepared task as a minimal, tested vertical slice** — tests first, inside the boundaries, code in the source tree.

## Process
1. **Branch + open the Verification Record.** Off main (`task/T-NNN-…`); never commit to main. Generate the pre-implementation checklist — `python3 scripts/check_task_record.py spec --init T-NNN` — which materializes, from the task's **frozen** spec references, the obligation table you'll be held to (`spec/10-delivery/verification/tasks/T-NNN.md`, every row `PENDING`). You see the bar before you write a line; you can't shrink it.
2. **UI slice? Open the visual contract before any test.** For a non-`N/A` `ux` dimension, load the **frozen prototype** + the `JRN-` journey's interaction states + the referenced `DS-` components/tokens + the `a11y` cell **first** — that set is the slice's second acceptance reference, held to like the ACs. Every state the `ux` cell names gets its failing test (state-tagged, like `@covers` for ACs); the keyboard/focus path and WCAG criteria from the `a11y` cell become assertions; styling flows only through the design system (a missing DS element is a `GAP … — design-system`, never bespoke-styled around).
3. **Tests first, layered.** From the acceptance criteria, write the failing tests across every layer the slice touches — unit (domain/logic), integration (adapters/persistence), contract (`API-`/`EVT-` schemas), e2e (the user-observable path). One behavior at a time, asserted through the public interface. **The task's cross-cutting cells are test obligations too:** every referenced `SEC-` deny rule / `ENTL-` gate gets its deny / over-limit / lapsed-billing test, every `obs`-cell signal (SLO- telemetry · EXP- analytics event) gets an emission assertion, an owned `DATA-` retention/deletion trigger gets exercised, an `ML-` capability gets its eval-harness run vs its bar — written with the slice, not deferred.
4. **Implement the minimal slice.** End-to-end (entrypoint → application → domain → persistence) to green. For an empirical unknown about the slice's shape, spike a throwaway prototype first.
5. **Handle edges just-in-time.** A behavior no task anticipated → stop, don't guess; record a `GAP` and resolve it (complete / `N/A` / accept a default as an ADR) before continuing.
6. **Ship the real deploy artifact for the slice's deployable surface.** For any deployable surface the slice adds (always for the walking skeleton), produce the **real** CI/deploy artifact that reaches the first environment of the ratified promotion path (per `infra-ops/cicd.md` + `topology.md`) — never an `echo`/`exit 0`/`# TODO`/`if: false` placeholder, never silently deferred "because infra isn't ready" (that's a `human-prereq` to escalate). Fill the `deploy` row PASS with the artifact path, or `N/A — no new deployable surface`.
7. **Run enforcers and ship.** Pre-commit runs the checks (incl. `check_no_fakes.py` + `check_no_skips.py` + `check_deploy_real.py` + `check_orphan_tests.py` — every behavioral test traces to a live spec driver) → conventional commit → open one MR/PR → CI → merge. On a re-run, re-touch only the impacted code.
...
- **The prototype is the acceptance reference for a UI slice — there is no draft tier.** Build the screen to the frozen prototype the way you build behavior to the ACs: every named interaction state implemented + state-tagged test, the design system as the only styling vocabulary, the a11y keyboard/focus path asserted, the layout holding at the DS breakpoints. Ship the evidence with the slice (the `ux:states` / `a11y` / `ux:rendered` rows of the Verification Record — an a11y scan with zero critical/serious, the breakpoint render run). An unstyled, states-missing, or off-design-system screen is a fake exactly like a stubbed adapter — green functional tests don't redeem it, and this binds from the **first** UI slice (the app shell included): feature 1 ships the fully-satisfied UI, not a placeholder to revisit.
- **A red suite over a real implementation is a valid way to end the task** — when a prerequisite (credential, env, external system) genuinely isn't ready, leave the real code and the real test in place, let the test fail, mark the task `blocked` with the escalation record, and move on. That outcome is reported as process-success; a green bought with a `skip`/`xfail`/`.only`, a mock in `src/`, or an e2e that avoids the not-ready path is the failure (`check_no_skips.py` + the exec-gate enforce this mechanically).
