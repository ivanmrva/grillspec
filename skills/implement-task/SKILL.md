...
6. **Run enforcers and ship.** Pre-commit runs the checks (incl. `check_no_fakes.py` + `check_no_skips.py` + `check_deploy_real.py`) → conventional commit → open one MR/PR → CI → merge. On a re-run, re-touch only the impacted code.
